...
#working progress 
psnu_int <- Drop3 %>% group_by("psnu","indicator") %>% filter((indicator == "OVC_SERV") & (categoryoptioncomboname == "Active" & categoryoptioncomboname == "Graduated"))
psnu <- unique(Drop3$psnu)

for (n in psnu){
  active_value <- if_else(exists(Drop3[ which((Drop3$psnu == n) & 
